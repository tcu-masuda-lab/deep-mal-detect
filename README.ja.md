...
このリポジトリでは下記の論文で示す実験内で作成した高精度のWindowsマルウェア検出モデル**DeepMalDetect**を公開しています。モデルの作成には[elastic/ember](https://github.com/elastic/ember)で公開されている`ember_dataset_2018`を使用しています。このDNNを用いたマルウェア検出モデルはハイパーパラメーターチューニングを通じて精度を向上させています。

本リポジトリで提供するプログラムはWindows上での利用を想定しています。
